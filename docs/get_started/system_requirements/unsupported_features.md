...
|Developing|HCL themes from a previous version|The themes  **Portal**, **PortalWeb2** and **Tab Menu - Page Builder** are no longer supported.<br> They no longer work and are no longer supported. You must manually update these themes. Merge their function into a clean copy of a Portal 8.5 theme on the target server. The PageBuilder2 and Portal 7002 themes are removed in 8.5.<br><br> Documentation resource: [Enabling a new functionality in a migrated portal](../deploy_dx/manage/migrate/next_steps/enable_func_migrated_portal/index.md)|
...
|Developing|CAI/TAI portlets|When you are migrating from one version to another, your script can contain references to the CAI/TAI portlets. These portlets are no longer available and any reference to these portlets cause your script to fail. For more information, see [Virtual Portal tasks](../deploy_dx/manage/migrate/next_steps/post_mig_activities/portal_task/vp_post_mig_task/index.md).|
...
|Developing|Business portlets|The following portlets **Reminder**, **Document Viewer**, **Webpage Portlet**, **My Query Reports** and **Microsoft Exchange 2003** were removed and are no longer supported.|
|Integrating|Sametime portlets|The Sametime portlets **Who Is Here** (SametimeWhoIsHere.war)   **Sametime Contact List** (SametimeContactList.war) were removed and are no longer supported.|
|Developing|Themes and Tags|The `<portal-core:cacheProxyUrl/>` tag was removed and is no longer supported: -   All Legacy themes|
...
|IBM Portlet API web content viewer portlet |The web content viewer portlet based on the IBM Portlet API is no longer supported. It was replaced with the JSR 286 web content viewer portlet.  |
|IBM Portlet API remote web content viewer portlet |The remote web content viewer portlet based on the IBM Portlet API is no longer supported. To display web content on a portal where Web Content Manager is not installed, use WSRP and the JSR 286 web content viewer.|
|IBM API Rendering Portlet  |The IBM API Rendering Portlet  is no longer supported.|
|Private wire APIs  |The WireModel artifacts referring to private wires is no longer supported.|