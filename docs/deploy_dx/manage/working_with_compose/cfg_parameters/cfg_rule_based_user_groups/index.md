...
The SoftGroups feature in HCL Digital Experience (DX) Compose lets you define dynamic portal user groups. These resources are defined by a unique group name, a Lightweight Directory Access Protocol (LDAP) search filter rule expression, and an optional description. DX Compose handles rule-based user groups as standard portal user groups that reside in a special base distinguished name within the user realm hierarchy.
...
Learn how to create the database table manually before using rule-based user groups.
...
Learn how to enable and configure the SoftGroups feature before using rule-based user groups.
